...
/*describe('auxiliaries performance', () => {
    ...
    it('should return the correct waiting time', () => {
        const mySpy = spy(console, 'log');
        //aux.logPerformanceStart('initialization');
        //await new Promise(res => setTimeout(res, 500));
        aux.logPerformanceStop('initialization', undefined);
        const printe = mySpy.getCall(0);
        mySpy.restore();
        console.log(printe);
    });
}); */
...
/*describe('auxiliaries get', () => {
    //const sandbox = sinon.createSandbox();

    beforeEach(() => {
        //sandbox.spy(console, 'log');
        //sandbox.spy(window, 'location');
    });
    ...
    it('search should return an empty string', () => {
        //const mySpy = spy(window, 'location');
        const window = stub();
        const search = aux.GETsearch();
        expect(typeof search).to.eq('string');

...
*webgl-operate* encapsulates WebGL and provides various rendering capabilites such as common rendering controllers for single-frame and multi-frame rendering, raycasting, or progressive path tracing as well as a multitude of common rendering processes and shaders. 
In contrast to most other rendering engines *webgl-operate* does not favor a specific rendering approach/pipeline or style nor does it assume a specific use-case. 
Instead, it provides a basis for arbitrary rendering implementations and simplifies most common rendering processes in an atomic maner.
It can be used for, e.g., complex charting, volume rendering, point-cloud rendering, product visualization, learning webgl, raymarching or pathtracing, rendering implicit or procedural geometry, or just common rendering of complex 3D scenes and models. 

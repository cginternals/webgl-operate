...
/**
 * Utility class providing simplified access to the clients fullscreen API. The toggle can be used to toggle fullscreen
 * for a HTML element while managing the execution of custom toggle code.
 *
 * ```
 * fullscreen_button.click(function () {
 *     ...
 *     softwaremap.Fullscreen.toggle(softwaremap_element.get(0), function () {
 *         softwaremap_element.toggleClass('fullscreen');
...
export class Fullscreen {
    ...
    /**
     * Event listener used to add the remove event listener using on indirection, that is, the first fullscreen event
     * triggered after a request is ignored. This is important for triggering the fullscreen callback before the
     * request is completed. The listener adds the removeListener and removes itself as listener.
     */
    ...
    static active(): boolean {
        return Fullscreen.element() !== undefined && Fullscreen.element() !== undefined;
    }

    /**
     * Requests or exits full screen mode for a given element. If the element is already in full screen, full screen
     * mode is exited. Else, full screen mode is requested. The function considers various platform specific full screen
     * interfaces, i.e., native, ms, moz, and webkit.
     *
     * @param element - Element to toggle full screen state of.
     */
    static toggle(element: HTMLElement, callback?: () => void): void {
        ...
        /**
         * The toggle callback is to be triggered first, always. Some browsers do not trigger a reflow when requesting
         * or exiting fullscreen and simultaneously toggling some HTML element classes.
         */
        if (callback) {
            callback();
        }
        ...
        /**
         * Requesting fullscreen and adding the addEL event listener to the fullscreenchange always triggers the event
         * listener on fullscreen request or exit. addEL then removes itself and adds the removeEL, which in turn
         * triggers the given callback and also removes itself as listener. This is a solid way to assure single
         * callback execution per toggle.
         */
        if (!isFullscreen) {

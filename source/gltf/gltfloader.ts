...
export class GLTFLoader {
    ...
    protected async loadTextures(asset: GltfAsset): Promise<void> {
        ...
        this._uriTextureMap = new Map<string, Texture2D>();
    ...
    protected async loadMeshes(asset: GltfAsset): Promise<Array<GLTFMesh>> {
        ...
        this._nameMeshMap = new Map<string, GLTFMesh>();
        const meshes = asset.gltf.meshes;
        ...
        let primitiveId = 0;
        for (const meshInfo of meshes) {
            const mesh = new GLTFMesh(meshInfo.name);
    ...
    protected async generateScenes(asset: GltfAsset): Promise<void> {
